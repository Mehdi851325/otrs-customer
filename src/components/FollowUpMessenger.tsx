...
import { useTicketUpdateMutation } from "../redux/features/api/apiSlice";
...
const FollowUpMessenger = ({ detailTicket, setShowReply, showSuccess }:Params) => {
  const [ticketUpdate] = useTicketUpdateMutation();
  const navigate = useNavigate();
  const { unit, id } = useParams<UrlParams>();
        ...
        <div>
          ...
          {errors.description && (
            <div className="pr-6 py-1 mb-8 bg-red-100 mt-1 rounded-md flex items-center">
              <BsExclamationOctagon color="red" />
              <p className="text-red-700 pr-2">{errors.description.message}</p>
            </div>
          )}
        </div>
        <div className="relative">
...
export default FollowUpMessenger;
